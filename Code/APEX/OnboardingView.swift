...
struct GoalStep: View {
    ...
    struct FocusOption: Identifiable {
        let id: String
        let label: String
        let groups: [Exercise.MuscleGroup]
        let icon: String
    }

    let focusOptions: [FocusOption] = [
        FocusOption(id: "chest",      label: "Chest",      groups: [.chest],             icon: "figure.strengthtraining.traditional"),
        FocusOption(id: "back",       label: "Back",       groups: [.back],              icon: "figure.pull.ups"),
        FocusOption(id: "shoulders",  label: "Shoulders",  groups: [.shoulders],         icon: "figure.arms.open"),
        FocusOption(id: "arms",       label: "Arms",       groups: [.biceps, .triceps],  icon: "dumbbell.fill"),
        FocusOption(id: "quads",      label: "Quads",      groups: [.quads],             icon: "figure.step.training"),
        FocusOption(id: "hamstrings", label: "Hamstrings", groups: [.hamstrings],        icon: "figure.run"),
        FocusOption(id: "glutes",     label: "Glutes",     groups: [.glutes],            icon: "figure.strengthtraining.functional"),
        FocusOption(id: "core",       label: "Core",       groups: [.core],              icon: "figure.core.training"),
    ]
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 28) {
                ...
                VStack(alignment: .leading, spacing: 10) {
                    HStack {
                        Text("Training Days").font(.headlineSmall).foregroundColor(.textSecondary)
                        Spacer()
                        Text("\(profile.trainingDays.count) days / week")
                            .font(.headlineSmall).foregroundColor(.accentMint)
                    }
                    HStack(spacing: 8) {
                        ForEach(Array(zip(1...7, ["M","T","W","T","F","S","S"])), id: \.0) { weekday, label in
                            let isOn = profile.trainingDays.contains(weekday)
                            Button {
                                if isOn {
                                    profile.trainingDays.removeAll { $0 == weekday }
                                } else {
                                    profile.trainingDays.append(weekday)
                                }
                                profile.workoutsPerWeek = profile.trainingDays.count
                            } label: {
                                Text(label)
                                    .font(.headlineSmall)
                                    .foregroundColor(isOn ? .black : .textSecondary)
                                    .frame(maxWidth: .infinity)
                                    .padding(.vertical, 10)
                                    .background(isOn ? Color.accentMint : Color.appCard)
                                    .clipShape(RoundedRectangle(cornerRadius: 10))
                            }
                        }
                    }
                    Text("Select your training days — the rest become rest days")
                        .font(.caption).foregroundColor(.textMuted)
                }

                VStack(alignment: .leading, spacing: 10) {
                    HStack {
                        Text("Priority Muscle Groups").font(.headlineSmall).foregroundColor(.textSecondary)
                        Spacer()
                        if !profile.focusMuscleGroups.isEmpty {
                            Button("Reset") { profile.focusMuscleGroups = [] }
                                .font(.caption).foregroundColor(.textMuted)
                        }
                    }
                    Text("Your plan adds more sessions for selected areas. Leave empty for a balanced split.")
                        .font(.caption).foregroundColor(.textMuted)
                    ...
                    LazyVGrid(
                        ...
                        ForEach(focusOptions) { option in
                            let isOn = option.groups.allSatisfy { profile.focusMuscleGroups.contains($0) }
                            Button {
                                if isOn {
                                    profile.focusMuscleGroups.removeAll { option.groups.contains($0) }
                                } else {
                                    for g in option.groups where !profile.focusMuscleGroups.contains(g) {
                                        profile.focusMuscleGroups.append(g)
                                    }
                                }
                            } label: {
                                VStack(spacing: 6) {
                                    Image(systemName: option.icon)
                                        .font(.system(size: 18))
                                        .foregroundColor(isOn ? .black : .accentMint)
                                    Text(option.label)
                                        .font(.caption.bold())
                                        .foregroundColor(isOn ? .black : .textSecondary)
                                }
