...
struct WorkoutView: View {
    ...
    @State private var activeDay: WorkoutDay?
                        ...
                        VStack(alignment: .leading, spacing: 14) {
                            ...
                            ForEach(plan.days.sorted { $0.weekday < $1.weekday }) { day in
                                WorkoutDayCard(day: day, isToday: isToday(weekday: day.weekday)) {
                                    if isToday(weekday: day.weekday) {
                                        activeDay = day
                                    } else {
                                        previewDay = day
                                    }
                    ...
                    HStack(alignment: .top) {
                        VStack(alignment: .leading, spacing: 6) {
                            Text(exercise.name)
                                .font(.headlineLarge).foregroundColor(.textPrimary)
                            HStack(spacing: 6) {
                                ForEach(exercise.muscleGroups.prefix(3), id: \.self) { m in
...
struct ExerciseGuideView: View {
    ...
    var figureSymbol: String {
        ...
        if exercise.muscleGroups.contains(.calves) { return "figure.step.training" }
        if exercise.muscleGroups.contains(.quads) || exercise.muscleGroups.contains(.hamstrings) || exercise.muscleGroups.contains(.glutes) {
            return "figure.strengthtraining.functional"
        }
        return "figure.strengthtraining.traditional"
    }

    var heroColor: Color { exercise.muscleGroups.first?.color ?? .accentMint }
                    ...
                    ZStack {
                        ...
                        VStack(spacing: 16) {
                            Image(systemName: figureSymbol)
                                ...
                                .scaleEffect(animating ? 1.1 : 0.92)
                                .animation(
                            ...
                            HStack(spacing: 20) {
                                ...
                                Label(exercise.reps + " reps", systemImage: "arrow.up.arrow.down")
                            ...
                            ForEach(Array(exercise.instructions.enumerated()), id: \.offset) { i, step in
                                HStack(alignment: .top, spacing: 14) {
                                    Text("\(i + 1)")
                                        ...
                                        .background(Circle().fill(heroColor))
                                    Text(step)
